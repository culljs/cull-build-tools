...
module.exports = function (sources, tests, template, target) {
    var sourceTokens = loadTokens(readAll(sources));
    var testContents = readAll(tests);
    var testTokens = loadTokens(testContents);

    var tpl = fs.readFileSync(template, "utf-8");
    var fns = publicFnList(sourceTokens);

    var result = tpl.
            replace("[[ function-index ]]", functionIndex(fns)).
        replace("[[ function-docs ]]", functionDocs(fns, sourceTokens, testTokens, testContents));
